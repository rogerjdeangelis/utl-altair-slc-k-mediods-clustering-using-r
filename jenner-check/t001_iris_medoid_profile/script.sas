/* From utl-altair-slc-k-mediods-clustering-using-r.sas by rogerjdeangelis.
   The iris DATA step (inline cards4 data), variable retain order, informat,
   and PROC SORT are the author's, unchanged. The only edit is the library
   reference (d:/wpswrkx WORKX -> WORK) so the step is self-contained; a
   PROC MEANS surfaces the per-species 4-dimensional profile the post analyzes. */
...
/* Per-species profile of the four measured dimensions (the space the
   k-medoids clustering operates on). */
proc means data=work.iris n mean min max maxdec=2;
 class Species;
